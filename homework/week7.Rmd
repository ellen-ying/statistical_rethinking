...
Use partial pooling to estimate the proportion of women using contraception in each district.
...
The model still gives an estimate for the missing district 54. It seems to assume that the mean and the sd of the missing district roughly equal to the mean and the sd of the population distribution.
...
The blue dots are empirical raw proportion of women using contraception, and the hollow dots are estimated proportions. The estimates shrink towards the population mean compared to the empirical proportion. The shrinkage is more pronounced in districts where the empirical data is far away from the estimated population mean.


### Problem 2

Draw a DAG of all variables and design an estimation strategy to identify both the total and direct causal effects of living in an urban center on contraceptive use.
...
To estimate the total effect, we need to control for D. To estimate the direct effect, we need to control for D, K, and A.


### Problem 3

Build models to estimate the total and direct causal effect of urban living on contraceptive use.
...
On average, living in urban places has positive effect on contraceptive use, which increases the log-odds by about 80%.

Stratify the effect of U by D. This is a model of varying intercepts and varying slopes:
...
In most districts, living in urban areas has no or positive effects on contraceptive use. In some districts, this has negative effects.

#### Direct effect

The model to estimate direct effect of urban living on contraception use:
...
Compute a marginal causal effect of urban living for each district, using standard age distribution.
...
Urban living has very small positive effects on contraceptive use and this effect is in most cases unreliable.
...
In stratifying the effect of urban living by districts, we need to model not only the variance among intercepts and slopes, but also the covariance between the two. The covariance part is what `m3.2` above did not do. (Was actually planning to do that but there was some problems with specifying the length of D vector while constructing a matrix of intercepts and slopes...very sloppy)
...
- Number of kids is an ordered category, so better model it as such (kids number cannot be continuous)
...
There are not much variation in the effect of K and A, as shown in the [3] and [4] elements of sigma.


### Problem 4

What the answer to P4 above missed was the impact of A and U on K, which is indicated by the DAG. To simulate the causal effect of changing U, we need to model this part in the statistical model as well.
...
The second thing the original answer missed was the age distribution. It is indeed from the original sample, but with some nuances:
